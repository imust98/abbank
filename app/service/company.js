 ...
 class CompanyService extends BaseService {
   async create(data) {
     const result = await this.app.mysql.insert('customer_company', data);
     return (result && {
       id: result.insertId
     }) || {};
   }
   async addCredit(pid, data) {
     ...
     if (result && result.affectedRows === 1) {
       // 客户表信息写入信用id
       const res = await this.app.mysql.update('customer_company', {
         'id': pid,
   ...
   async delete(id) {
     const result = await this.app.mysql.delete('customer_company', {
       id: id
     });
   ...
   async update(id, data) {
     ...
     const result = await this.app.mysql.update('customer_company', data);
     return (result && {
       id
     }) || {};
   }
     ...
     if (params.type) {
       if (params.keyword) {
         if (params.type === 'credit') {
           sql = `select * from customer_company  where credit_id and ${params.keyword} = '${params.value}'`;
         } else {
           sql = `select * from customer_company  where loans_count and ${params.keyword} = '${params.value}'`;
         }
       } else {
         if (params.type === 'credit') {
           sql = `select * from customer_company  where credit_id`;
         } else {
           sql = `select * from customer_company  where loans_count`;
         }
       }
     } else {
       if (params.keyword) {
         if (params.type === 'credit') {
           sql = `select * from customer_company  where  ${params.keyword} = '${params.value}'`;
         } else {
           sql = `select * from customer_company  where  ${params.keyword} = '${params.value}'`;
         }
       } else {
   ...
   async getItem(id) {
     const result = await this.app.mysql.get('customer_company', {
       id
     });
     if (result.credit_id) {
